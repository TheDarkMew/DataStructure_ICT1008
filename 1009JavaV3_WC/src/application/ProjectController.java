...
public class ProjectController extends Application implements Initializable {
	...
	@FXML
	private TableView<testTable> twittertableView;
	@FXML
	private TableView<testRedditTable> reddittableView;
	@FXML
	private TableColumn<testTable, String> userNameColumn;
	@FXML
	private TableColumn<testTable, Integer> retweetsColumn;
	@FXML
	private TableColumn<testTable, Integer> favouritesColumn;
	@FXML
	private TableColumn<testTable, String> postColumn;
	@FXML
	private TableColumn<testTable, String> datesColumn;
	@FXML
	private TableColumn<testTable, String> sentimentColumnT;
	@FXML
	...
	@FXML
	private TableColumn<testRedditTable, String> titleColumn;
	@FXML
	private TableColumn<testRedditTable, Integer> scoreColumn;
	@FXML
	private TableColumn<testRedditTable, String> dateColumn;
	@FXML
	private TableColumn<testRedditTable, String> authorColumn;
	@FXML
	private TableColumn<testRedditTable, String> subRedditColumn;
	@FXML
	private TableColumn<testRedditTable, String> sentimentColumnR;
	@FXML
	...
	public ObservableList<testTable> readCSV() {
		...
		for (TwitterPost post : tpost) {
			testTable twitterTable = new testTable(post.getUsername(), post.getRTCount(), post.getLikeCount(),
					post.getPostContent(), post.getPostDate(), post.getSentiment());
	...
	public ObservableList<testRedditTable> readCSV2() {
		...
		for (RedditPost post : rpost) {
			testRedditTable redditTable = new testRedditTable(post.getPostContent(), post.getLikeCount(),
					post.getPostDate(), post.getUsername(), post.getPostSubreddit(), post.getSentiment());
			dataList1.add(redditTable);
			for (RedditComment comment : post.getCommentList()) {
				testRedditTable redditTable2 = new testRedditTable(comment.getCommentText(), comment.getCommentScore(),
						comment.getCommentDate(), comment.getUsername(), "From Above Post", comment.getSentiment());
	...
	public void setBarChart() {
		...
		int[] redditStats = a.getRedditStats(); // total posts, total comments, total post upvotes, total comment
												// upvotes
	...
	public void setPieChart() {
		...
		ObservableList<PieChart.Data> pieChartData = FXCollections.observableArrayList(
				new PieChart.Data("Very Positive", sentimentStats[0]), new PieChart.Data("Positive", sentimentStats[1]),
				new PieChart.Data("Neutral", sentimentStats[2]), new PieChart.Data("Negative", sentimentStats[3]),
	...
	public void displayWordCloud() {
		try {
			wcimage.setImage(new Image(new FileInputStream("./output/KobeWordCloud.png")));
		} catch (Exception e) {
	...
	// initialize columns with variables as cells from testTable.java and
	// testRedditTable.java
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		alert.setContentText(
				"Warning: Crawling for data will take a few minutes. Please be patient.\nImporting data would require an existing reddit.csv and twitter.csv.");
		...
		alert3.setContentText("Loading complete!!");
...
//			alert2.setContentText("Please wait!");
//
//			alert2.showAndWait();
//			Platform.runLater(() -> {
//				Alert alert3 = new Alert(AlertType.INFORMATION);
